...
void LinkedList<T>::insertOrdered(const T &newData)
{
    ...
    Node *curBig = head_; // base (assuming that it's bigger than new Data)
    Node *curSmall = nullptr;    // for previous data for newData
    
    // traverse the 
    while (curBig)
    {
        if (curBig->data > newNode->data)
        {
            curBig->prev = newNode;
            if (curSmall != nullptr)
            {
                curSmall->next = newNode;
                newNode->prev = curSmall;
            }
            else
            {
                head_ = newNode;
        ...
        // if there's no node bigger than newData
        if (curBig == tail_)
        {
            curBig->next = newNode;
            newNode->prev = curBig;
    ...
    // if the calling list is empty, make newNode as first node of the list 
    if (curBig == nullptr) {
        curBig = newNode;
        head_ = newNode;
        tail_ = newNode;
